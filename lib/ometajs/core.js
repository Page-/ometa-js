(function (root, factory) {
	...
	*/
	...
	return OMeta = {
		...
		// match, matchAll and createInstance are a grammar's "public interface"
		match: function(obj, rule, args, matchFailed) {
			return this.matchAll([obj], rule, args, matchFailed);
		},
		matchAll: function(listyObj, rule, args, matchFailed) {
			var m = this._createInstance();
			m.setInput(listyObj);
			m.initialize();
			return m._matchAll(rule, args, matchFailed);
		},
		createInstance: function() {
			var m = this._createInstance();
			m.matchAll = function(listyObj, rule, args, matchFailed) {
				this.setInput(listyObj);
				return this._matchAll(rule, args, matchFailed)
			}
			m.initialize();
			return m;
		},
		...
		_createInstance: function() {
			...
			m._matchAll = function(rule, args, matchFailed) {
				if (args == null) {
					args = [];
				}
				var realArgs = [rule];
